...
from modules.realtime_detection.model_object_detect_midle_magnet_triton import Detect_Middle_Magnet
...
from modules.realtime_detection.model_object_detect_phone_on_slot_triton import Object_Detection_YOLOv8_one_phone
from modules.realtime_detection.model_object_detect_phone_on_gripper import Object_Detection_YOLOv8_phone_on_gripper
...
class AnalyzeController(QThread):
    ...
    def __init__(self, general_model: GeneralModel):
        ...
        # self.logtime = logging.getLogger(__name__)
    ...
    def run(self):
        ...
        model_object_detect_phone_exception_box_triton_weight_path = "./model_object_detect_phone_exception_box.onnx"
        model_object_detect_magnetic_exception_box_triton_weight_path = "./model_object_detect_magnetic_exception_box.onnx"
        model_object_detect_magnetic_triton_weight_path = "./model_object_detect_magnetic_yolov8.onnx"
        model_object_detect_phone_on_gripper_triton_weight_path = "./model_object_detect_phone_on_gripper_yolov8.onnx"
        ...
        labelmap_path_model_object_detect_phone_exception_box_triton = "/home/greystone/StorageWall/model_template/Model_Triton/model_object_detect_phone_exception_box/label.pbtxt"
        labelmap_path_model_object_magnetic_phone_exception_box_triton = "/home/greystone/StorageWall/model_template/Model_Triton/model_object_detect_magnetic_exception_box/label.pbtxt"
        ...
        labelmap_path_model_object_detect_phone_on_gripper_triton = "/home/greystone/StorageWall/model_template/Model_Triton/model_object_detect_phone_on_gripper_yolov8/label.pbtxt"
        slot_id = None
        list_object_magnetic = None
        ...
        try: 
            phone_exception_box_detector_triton = Object_Detection_YOLOv8_one_phone(model_object_detect_phone_exception_box_triton_weight_path, labelmap_path_model_object_detect_phone_exception_box_triton, 0.7,  list_object_magnetic)
            self.log.info('[ANALYZE_CONTROLER] Load model phone_exception_box_detector_triton successfuly!')
        except Exception as e:
            self.log.info(f'[ANALYZE_CONTROLER] FAIL to Load model phone_exception_box_detector_triton, Error = {e}')
            
        try: 
            magnetic_exception_box_detector_triton = Detect_Middle_Magnet(model_object_detect_magnetic_exception_box_triton_weight_path,labelmap_path_model_object_magnetic_phone_exception_box_triton, 0.70 )
            self.log.info('[ANALYZE_CONTROLER] Load model magnetic_exception_box_detector_triton successfuly!')
        except Exception as e:
            self.log.info(f'[ANALYZE_CONTROLER] FAIL to Load model magnetic_exception_box_detector_triton, Error = {e}')
            
        try:
            phone_slot_object_detector_one_phone_triton = Object_Detection_YOLOv8_one_phone(model_object_detect_phone_slot_triton_weight_path, labelmap_path_model_object_detect_phone_slot_triton, 0.83,  list_object_magnetic)
            self.log.info('[ANALYZE_CONTROLER] Load model phone_slot_object_detector_one_phone_triton successfuly!')
        except Exception as e:
            self.log.info(f'[ANALYZE_CONTROLER] FAIL to Load model phone_slot_object_detector_one_phone_triton, Error = {e}')
        
        try:
            phone_on_gripper_object_detector_triton = Object_Detection_YOLOv8_phone_on_gripper(model_object_detect_phone_on_gripper_triton_weight_path, labelmap_path_model_object_detect_phone_on_gripper_triton, 0.75)
            self.log.info('[ANALYZE_CONTROLER] Load model phone_on_gripper_object_detector_triton successfuly!')
        except Exception as e:
            self.log.info(f'[ANALYZE_CONTROLER] FAIL to Load model phone_on_gripper_object_detector_triton, Error = {e}')
        ...
        try:
            object_detect_magnetic_middle_triton = Detect_Middle_Magnet(model_object_detect_magnetic_triton_weight_path,labelmap_path_model_object_detect_magnetic_triton, 0.70 )
            self.log.info('[ANALYZE_CONTROLER] Load model phone_slot_object_detector_middle_triton successfuly!')
        except Exception as e:
            self.log.info(f'[ANALYZE_CONTROLER] FAIL to Load model phone_slot_object_detector_middle_triton, Error = {e}')
        ...
        while not self._exit_flag:
            ...
            if request_type == 'check_phone_on_gripper' or request_type == 'detect_phone_slot' or request_type == 'detect_phone_slot_fast' or request_type == 'calibrate' or request_type == 'scan_barcode' or request_type == 'check_device_slot_status':
                ...
                if transaction_id != "":                
                    ...
                    final_processing_images_path = os.path.join(GV().path_to_debug_folder,"final_processing_images")
                ...
                if request_type == "detect_phone_slot":
                    try:                  
                        ...
                        debug_img, position_have_phone = image_detect_phone, (None, None, None)
                        ...
                        elif positionToDetect == 'left' or positionToDetect == 'right':
                            result_gripper_status, debug_img, list_object_magnetic = object_detect_magnetic_middle_triton.run(path_image, image_detect_phone, positionToDetect, ml_debug_path)
                            result_slot_status, debug_img, result_phone_status = phone_slot_object_detector_one_phone_triton.run(path_image, positionToDetect, numberSlotsDetector, debug_img, list_object_magnetic, ml_debug_path)
                        elif positionToDetect == 'left_exception' or positionToDetect == 'right_exception' or positionToDetect == 'center_exception':
                            result_gripper_status, debug_img, list_object_magnetic = magnetic_exception_box_detector_triton.run(path_image, image_detect_phone, positionToDetect, ml_debug_path)
                            result_slot_status, debug_img, result_phone_status = phone_exception_box_detector_triton.run(path_image, positionToDetect, numberSlotsDetector, debug_img, list_object_magnetic, ml_debug_path)
                        ...
                        #     result_gripper_status, debug_img, list_object_magnetic = object_detect_magnetic_middle_triton.run(image_detect_phone, image_detect_phone, positionToDetect, ml_debug_path)
                        #     result_slot_status, debug_img, result_phone_status = phone_slot_object_detector_one_phone_triton.run(path_image, positionToDetect, numberSlotsDetector, debug_img, list_object_magnetic, ml_debug_path)
                        ...
                        #     result_phone_status, debug_img, list_object_magnetic = object_detect_magnetic_3slots_triton.run(image_detect_phone, debug_img, positionToDetect, ml_debug_path, numberSlotsDetector, position_have_phone)
                            
                        #     print(list_object_magnetic)
                        #     result, debug_img, position_phone_center, value_deviation = phone_slot_object_detector_triton.run(path_image, positionToDetect, list_object_magnetic)
                        ...
                        if numberSlotsDetector == 3:
                            result_dict["slot_1"] = {"slot_status": None, "phone_status": None}
                            result_dict["slot_2"] = {"slot_status": None, "phone_status": None}
                            result_dict["slot_3"] = {"slot_status": None, "phone_status": None}
                            result_dict["slot_1"]["slot_status"] = result_slot_status[0]
                            result_dict["slot_2"]["slot_status"] = result_slot_status[1]
                            result_dict["slot_3"]["slot_status"] = result_slot_status[2]
                            result_dict["slot_1"]["phone_status"] = result_phone_status[0]
                            result_dict["slot_2"]["phone_status"] = result_phone_status[1]
                            result_dict["slot_3"]["phone_status"] = result_phone_status[2]
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                if request_type == "check_device_slot_status":
                    result_dict = dict()
                    try:
                        self.log.info("[CHECK_DEVICE_SLOT_STATUS] START cmdUniqueID = {} path_image= {} path_debug = {}".format(cmdUniqueID,path_image,path_debug))
                        image_origin = None
                        image_detect_phone = cv2.imread(path_image)
                        ...
                        if image_detect_phone is not None:
                            ...
                            if( positionToDetect == 'left' or positionToDetect == 'right'):
                                result_gripper_status, debug_img, list_object_magnetic = object_detect_magnetic_middle_triton.run(image_detect_phone, image_origin, positionToDetect, ml_debug_path)
                                result, debug_img, phone_status = phone_slot_object_detector_one_phone_triton.run(path_image, positionToDetect, numberSlotsDetector, debug_img, list_object_magnetic, ml_debug_path)
                            elif( positionToDetect == 'left_exception' or positionToDetect == 'right_exception' or positionToDetect == 'center_exception'):
                                result_gripper_status, debug_img, list_object_magnetic = magnetic_exception_box_detector_triton.run(image_detect_phone, image_origin, positionToDetect, ml_debug_path)
                                result, debug_img, phone_status = phone_exception_box_detector_triton.run(path_image, positionToDetect, numberSlotsDetector, debug_img, list_object_magnetic, ml_debug_path)
                            if 'have_phone' in result:
                                self.log.info("[CHECK_DEVICE_SLOT_STATUS] END cmdUniqueID = {} result = HAVE_PHONE path_image= {} ml_debug_path = {}".format(cmdUniqueID, path_image, ml_debug_path))
                                result_dict["slot_status"] = "have_phone"
                            else:
                                result_dict["Slot_status"] = "none_phone"
                                
                            result_dict["gripper_status"] = {
                                "leftDeviated": result_gripper_status["left"],   # 0
                                "rightDeviated": result_gripper_status["right"],  # 1
                                "topDeviated": result_gripper_status["top"],     # 2
                                "botDeviated": result_gripper_status["bot"],     # 3
                                "cwDeviated": result_gripper_status["cw"],    # 4
                                "ccwDeviated": result_gripper_status["ccw"]    # 4
                            }
                            result_dict["phone_status"] = phone_status
                            module_info.response.result_string = result_dict
                                
                            self.log.info("[CHECK_DEVICE_SLOT_STATUS] END cmdUniqueID = {} result ={} path_image= {} ml_debug_path = {}".format(cmdUniqueID,result_dict, path_image, ml_debug_path))
                            
                        else:
                            self.log.info(f"[CHECK_DEVICE_SLOT_STATUS] Image detect phone slot from SW is Null ==>ERROR \n")
                        # Copy log file total to debug folder
                        path_backup_log_phone_slot = os.path.join(ml_debug_path,"log_phone_slot.log")
                        cmd = f"cp -rf {os.path.join(GV().path_to_logs_folder,'log_file.log')} {path_backup_log_phone_slot}"
                        self.log.info(f"[CHECK_DEVICE_SLOT_STATUS] cmd backup log: {cmd}")
                        os.system(cmd)             

                    except Exception as e:
                        self.log.info("[CHECK_DEVICE_SLOT_STATUS] FAIL to execute cmdUniqueID = {} path_image = {} path_debug = {} Error = {}".format(cmdUniqueID,path_image,path_debug,e))

# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                if request_type == "check_phone_on_gripper":
                    result_dict = dict()
                    try:
                        self.log.info("[CHECK_POSITION_PHONE_ON_GRIPPER] START cmdUniqueID = {} path_image= {} path_debug = {}".format(cmdUniqueID,path_image,path_debug))
                        image_origin = None
                        image_detect_phone = cv2.imread(path_image)
                        ...
                        if image_detect_phone is not None:
                            ...
                            result_gripper_status, debug_img, list_object_magnetic = object_detect_magnetic_middle_triton.run(image_detect_phone, image_origin, positionToDetect, ml_debug_path)
                            result, debug_img, phone_status = phone_slot_object_detector_one_phone_triton.run(path_image, positionToDetect, numberSlotsDetector, debug_img, list_object_magnetic, ml_debug_path)
                            result_phone_on_gripper, debug_img_phone_on_gripper = phone_on_gripper_object_detector_triton.run(path_image, debug_img, ml_debug_path)
                            if 'have_phone' in result:
                                self.log.info("[CHECK_POSITION_PHONE_ON_GRIPPER] END cmdUniqueID = {} result = HAVE_PHONE path_image= {} ml_debug_path = {}".format(cmdUniqueID, path_image, ml_debug_path))
                                result_dict["slot_status"] = "have_phone"
                            ...
                            result_dict["phone_status"] = phone_status
                            result_dict["phone_on_gripper"] = result_phone_on_gripper
                            module_info.response.result_string = result_dict
                            
                            self.log.info("[CHECK_POSITION_PHONE_ON_GRIPPER] END cmdUniqueID = {} result ={} path_image= {} ml_debug_path = {}".format(cmdUniqueID,result_dict, path_image, ml_debug_path))
                            
                        else:
                            self.log.info(f"[CHECK_POSITION_PHONE_ON_GRIPPER] Image detect phone on gripper from SW is Null ==>ERROR \n")
                        # Copy log file total to debug folder
                        path_backup_log_phone_slot = os.path.join(ml_debug_path,"log_phone_slot.log")
                        cmd = f"cp -rf {os.path.join(GV().path_to_logs_folder,'log_file.log')} {path_backup_log_phone_slot}"
                        self.log.info(f"[CHECK_POSITION_PHONE_ON_GRIPPER] cmd backup log: {cmd}")
                        os.system(cmd)             

                    except Exception as e:
                        self.log.info("[CHECK_POSITION_PHONE_ON_GRIPPER] FAIL to execute cmdUniqueID = {} path_image = {} path_debug = {} Error = {}".format(cmdUniqueID,path_image,path_debug,e))
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                if request_type == "detect_phone_slot_fast":
                    ...
                    try:
                        ...
                        result_calib, image_pad_debug_1, image_pad_debug_2, offset_reference = calib_magnet_detector.run(path_image_1, path_image_2, position_to_calib, path_log_id_phone_slot, ml_debug_path, offset_reference)
            ...
            if request_type == 'detect_phone_slot' or request_type == 'detect_phone_slot_fast' or request_type == 'calibrate' or request_type == 'scan_barcode' or request_type == "check_device_slot_status" or request_type == 'check_phone_on_gripper':
