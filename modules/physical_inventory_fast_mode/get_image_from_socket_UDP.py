...
class GetImageFromSocketUDP:
    ...
    def subprocess_get_image_from_UDP(self, share_buffer_queue, count_total, count_fail, fps_get_img):
        ...
        while not self.isExitApp:
            ...
            if received_bytes == 12:
                total_pack = struct.unpack("B", buffer[:4])[0]
                size = struct.unpack("B", buffer[4:8])[0]
                crc32 = struct.unpack("B", buffer[8:12])[0]
...
if __name__ == "__main__":
    ...
    process_get_img = multiprocessing.Process(target = get_image.subprocess_get_image_from_UDP, args = (share_buffer_queue, count_total, count_fail, fps_get_img))
