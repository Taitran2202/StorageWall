...
class SegmentPlasticBox:
    def __init__(self, model_path, conf_thres=0.6, iou_thres=0.6, num_masks=32):
    ...
    def draw_detections(self, image, object_list, mask_alpha=0.3, mask_maps=None):
        ...
        mask_img = self.draw_masks(image, object_list, mask_alpha, mask_maps)
    ...
    def draw_masks(self, image, object_list, mask_alpha=0.3, mask_maps=None):
        ...
        for i, (box, class_id, score) in enumerate(zip(boxes, class_ids, scores)):
            ...
            if mask_maps is None:
                ...
                cv2.putText(mask_img, str(round(score,2)), (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    ...
    def post_process(self, object_list, debug_img, text_log_file):
        ...
        if len(object_list) == 0:
            ...
            combined_img = self.draw_detections(debug_img, object_list)
...
if __name__ == '__main__':
    ...
    path_log_id_phone_slot = "/home/greystone/StorageWall/image_debug/image_origin/log_id_phone_slot.txt"
    model_phone_slot_weight_path = '/home/greystone/StorageWall/model_template/PhoneSlots/model_segment_detect_phone_slot_BK.onnx'
    ml_debug_path = "/home/greystone/StorageWall/image_debug/image_predict"
    plastic_box_detector = SegmentPlasticBox(model_phone_slot_weight_path, conf_thres=0.5, iou_thres=0.5)
    
    img_tail = ['jpg','png','jpeg']
    image_folder_path = "/home/greystone/StorageWall/image_debug/image_origin"
    items = os.listdir(image_folder_path)
    for item in tqdm(items):
        item_tail = item.split('.')[-1]
        if item_tail in img_tail:
            image_path = os.path.join(image_folder_path, item)
            result, debug_img = plastic_box_detector.run(image_path, path_log_id_phone_slot)
            cv2.imwrite(os.path.join(ml_debug_path,item),debug_img)
