...
class YOLOv8_Triton:
    def __init__(self, model_path, labelmap_path, threshold) -> None:
        ...
        self.status_phone_slot_left_wall = np.zeros((54,24),np.uint8)
        self.status_phone_slot_right_wall = np.zeros((54,24),np.uint8)
        self.count_status_phone_slot_left_wall = np.zeros((54,24),np.uint8)
        self.count_status_phone_slot_right_wall = np.zeros((54,24),np.uint8)
    ...
    def _load_model_ready(self):
        ...
        try:
            ...
            logging.exception(f"Exception in function _load_model_ready = {e}")
...
class Object_Detection_Physical_Inventory_YOLOv8(YOLOv8_Triton):
    ...
    def detect(self, image):
        logging.info('In function detect!')
        # Ping to model on triton
        logging.info('model_name_on_triton: {}'.format(self.model_name_on_triton))
        is_model_ready = self._load_model_ready()
        if is_model_ready is False:
            logging.warning("triton model {} is not ready; exit".format(self.model_name_on_triton))
            return [], []
        logging.info('Triton model is ready')
        # Preprocessing image
        h, w = image.shape[:2]
        ...
        logging.info('Quantity object in image: {}'.format(len(detections)))
        ...
        logging.info('End function detect!')
        return bboxes
    ...
    def post_process(self, original_image, bboxes, positionToDetect, position_x_cur, position_y_cur):
        ...
        logging.info('In function post-proceess!')
        phone_list = []
        debug_img = original_image
        
        if positionToDetect == 'left':
            logging.info('Open file config for detect the phone in the left wall!')
            ...
            logging.info('Open file config for detect the phone in the right wall!')
            with open('/home/greystone/StorageWall/apps/ml-subapp/appPython/config.json', 'r') as json_file:
                data = json.load(json_file)
            ...
            logging.info('Open file config for detect the phone in the buffer!')
            with open('/home/greystone/StorageWall/apps/ml-subapp/appPython/config.json', 'r') as json_file:
                data = json.load(json_file)
            ...
            logging.info('Open file config for detect the phone in the Input dock!')
            with open('/home/greystone/StorageWall/apps/ml-subapp/appPython/config.json', 'r') as json_file:
                data = json.load(json_file)
            ...
            logging.info('ERROR: Require key positionToDetect from SW!')
        ...
        cv2.putText(debug_img, msg, (0,200), cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 0, 255], 2)
        cv2.putText(debug_img, f'R{slot_y_cur}C{slot_x_cur}', (0,250), cv2.FONT_HERSHEY_SIMPLEX, 2, [0, 255, 0], 2)

        if is_has_phone:
            result = self.result_have_phone
            logging.info('Found overlapped phone. Return have_phone')
        else:
            result = self.result_none_phone
            logging.info('Not found overlapped phone. Return no_phone')
    
        return result, slot_x_cur, slot_y_cur, debug_img

    def run(self, share_buffer_image_queue, positionToDetect, share_buffer_offset_queue):
        # init logger
        while True:
            fps = 0
            ...
            try:
                ...
                # Detect
                logging.info('Start run function detect!')
                object_list = self.detect(original_image)     
                logging.info('End run function detect!')        
                
                # Print out current object
                logging.info('Current object list: {object_list}')  
                
                # post process
                logging.info('Start run function post-process!')  
                ...
                logging.info('End run function post-process!')  
...
if __name__ == "__main__":
    ...
    video_path = "/home/greystone/StorageWall/image_debug/physical_inventory/recording_2024-05-11.avi"
    
    #warm up
    black_image = np.zeros((600,1200,3),np.uint8)
    result, slot_x_cur, slot_y_cur, image_debug = model.run(black_image, log_file_path, positionToDetect, 0, 0)
    
    status_phone_slot_left_wall = np.zeros((54,24),np.uint8)
    status_phone_slot_right_wall = np.zeros((54,24),np.uint8)
    count_status_phone_slot_left_wall = np.zeros((54,24),np.uint8)
    count_status_phone_slot_right_wall = np.zeros((54,24),np.uint8)
    ...
    while True:
        try:
            ...
            #     break
            count += 1
            image = get_image.get_1_image_streaming()
            ...
            position_x_cur, position_y_cur = get_data.get_1_offset_streaming()
            if (position_x_cur is None) or (position_y_cur is None):
                print(f'position_cur {position_x_cur}, {position_y_cur}')
                continue
            else:
                result, slot_x_cur, slot_y_cur, image_debug = model.run(image, log_file_path, positionToDetect, position_x_cur, position_y_cur)
                print("result: ", result)
                if result == 'have_phone':
                    if (positionToDetect == 'left') and (slot_x_cur in range(1,25)) and (slot_y_cur in range(1,55)):
                        count_status_phone_slot_left_wall[slot_y_cur-1][slot_x_cur-1] += 1
                    elif (positionToDetect == 'right') and (slot_x_cur in range(1,25)) and (slot_y_cur in range(1,55)):
                        count_status_phone_slot_right_wall[slot_y_cur-1][slot_x_cur-1] += 1
                print(result)
                print(count)
                if image_debug is not None:
                    cv2.imwrite(f"/home/greystone/StorageWall/image_debug/img_debug/PI_debug_{count}.png", image_debug)
            
            position_y_cur += 3.47
            print("position_y_cur", position_y_cur)
    ...
    for column in range(24):
        for row in range(54):
            if count_status_phone_slot_left_wall[row][column] > 3:
                status_phone_slot_left_wall[row][column] = 1
            if count_status_phone_slot_right_wall[row][column] > 3:
                status_phone_slot_right_wall[row][column] = 1
    status_phone_slot_left_wall_to_list = status_phone_slot_left_wall.tolist()
    with open("/home/greystone/StorageWall/image_debug/physical_inventory/result.json", "w") as json_file:
