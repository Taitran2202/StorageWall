...
class Object_Detection_YOLOv8_phone_on_gripper(YOLOv8_Triton):
    ...
    def detect(self, image):
        ...
        for bbox in detections:
            class_name = self.cls_names.get(int(bbox[5])+2)
    ...
    def post_process(self, image, bboxes):
        with open('/home/greystone/StorageWall/apps/ml-subapp/appPython/config.json', 'r') as json_file:
            data = json.load(json_file)
        tilted_downward = data.get("position_tilted_downward")
        tilted_upward = data.get("position_tilted_upward")
        debug_img = image
        
        xmin, ymin, xmax, ymax, cls_name, score = bboxes[0]
        result = 'No_tilted'
        if ymin < tilted_upward:
            self.log.info('Return phone on gripper is tilted upward !!!!!!!!!!!!!!')
            result = 'tilted_upward'
        if ymin > tilted_downward:
            self.log.info('Return phone on gripper is tilted downward !!!!!!!!!!!!!!')
            result = 'tilted_downward'
        
        return result

    def run(self, image_path, debug_img,  ml_debug_path):
        try:
            result = {}
            ...
            self.log.info("Time processing detect phone:{} ".format(len(object_list)))
            ...
            result = self.post_process(original_image, object_list)
            ...
            cv2.imwrite(os.path.join(ml_debug_path, "image_position_phone_on_gripper.png"), debug_img)
            # Close file
...
if __name__ == "__main__":
    ...
    threshold = 0.7
    model = Object_Detection_YOLOv8_phone_on_gripper(model_path, labelmap_path, threshold)
