import logging
from PyQt5.QtCore import QObject, QDataStream, qChecksum, QIODevice, QTimer, \
...
class ClientSocket(QTcpSocket):
    ...
    def __init__(self, parent=None):
        ...
        self._retry_timer = QTimer(self)  # Timer for retrying connection
        self._retry_timer.setInterval(10000)  # Retry every 10 seconds
        self._retry_timer.timeout.connect(self._try_connect_to_server)
    ...
    def connect_to_server(self):
        if not self.isOpen():
            self.log.info("Attempting to connect to {}:{}".format(self._host_info, self._port_info))
            self._try_connect_to_server()
        else:
            self.log.info("Open status to port {}:{}".format(self._host_info, self._port_info))

    def _try_connect_to_server(self):
        self.connectToHost(self._host_info, self._port_info)
        # Check connection status after a short delay
        QTimer.singleShot(1000, self._check_connection_status)

    def _check_connection_status(self):
        if self.state() == QTcpSocket.ConnectedState:
            self.log.info("Successfully connected to {}:{}".format(self._host_info, self._port_info))
            self._retry_timer.stop()
        else:
            self.log.info("Failed to connect to {}:{}, will retry...".format(self._host_info, self._port_info))
